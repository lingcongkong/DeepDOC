...
class GradCAM:
    ...
    def get_target_width_height(input_tensor):
        width, height, depth = input_tensor.size(-1), input_tensor.size(-2), input_tensor.size(-3) #这种写法很有趣，可以记录一下
...
#                 img = np.resize(img, target_size) #注意：cv2.resize(src, (width, height))，width在height前，格式应注意
            result.append(img)
...
#         return cam_per_layer,output, loss #将一整个列表的在channels维度求平均压缩，该维度处理后为1
        return self.aggregate_multi_layers(cam_per_layer), cam_per_layer #将一整个列表的在channels维度求平均压缩，该维度处理后为1
